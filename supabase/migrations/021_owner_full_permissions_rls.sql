...
CREATE POLICY tables_insert ON tables
    ...
    WITH CHECK (
        ...
        AND auth.user_role() IN ('manager', 'owner')
    );
...
CREATE POLICY tables_update ON tables
    ...
    USING (
        outlet_id = auth.user_outlet_id()
        AND auth.user_role() IN ('manager', 'staff', 'cashier', 'owner')
    )
    WITH CHECK (
        outlet_id = auth.user_outlet_id()
        AND auth.user_role() IN ('manager', 'staff', 'cashier', 'owner')
...
CREATE POLICY categories_update ON categories
    FOR UPDATE
    USING (outlet_id = auth.user_outlet_id() AND auth.user_role() IN ('manager', 'owner'))
    WITH CHECK (outlet_id = auth.user_outlet_id() AND auth.user_role() IN ('manager', 'owner'));
...
CREATE POLICY ingredients_select ON ingredients
    ...
    USING (
        outlet_id = auth.user_outlet_id()
        AND auth.user_role() IN ('manager', 'warehouse', 'owner')
    );
...
CREATE POLICY recipes_select ON recipes
    ...
    USING (
        ...
        AND auth.user_role() IN ('manager', 'warehouse', 'staff', 'cashier', 'owner')
    );
...
CREATE POLICY bills_insert ON bills
    ...
    WITH CHECK (
        outlet_id = auth.user_outlet_id()
        AND auth.user_role() IN ('manager', 'cashier', 'owner')
    );
...
CREATE POLICY printers_update ON printers
    FOR UPDATE
    USING (outlet_id = auth.user_outlet_id() AND auth.user_role() IN ('manager', 'cashier', 'owner'))
    WITH CHECK (outlet_id = auth.user_outlet_id() AND auth.user_role() IN ('manager', 'cashier', 'owner'));
