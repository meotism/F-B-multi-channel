// Edge Function: Inventory Restoration on Order Cancel
// Atomically restores ingredient stock based on recipe formulas when an
// order is cancelled. Uses a dedicated stored procedure with FOR UPDATE
// row-level locking to ensure consistent inventory state in concurrent
// scenarios.
//
// POST /functions/v1/restore-inventory
// Body: { order_id: string, outlet_id: string }
// Returns: 200 with restorations[]
//
// Requirements: 5.2 AC-10 (restore inventory on cancel)
// Design reference: Section 4.6.7
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { order_id, outlet_id } = body as {
      order_id: string;
      outlet_id: string;
    };
    ...
    // 7c. Verify the caller belongs to the requested outlet (security check)
    ...
    // 8. Call the stored procedure for atomic inventory restoration.
    //    The procedure handles: order validation, ingredient aggregation,
    //    FOR UPDATE locking, inventory increment, and audit log creation
    ...
    if (rpcError) {
      ...
      console.error('restore-inventory rpc error:', JSON.stringify({
    ...
    // 10. Return success with restoration details
    return jsonResponse({
    ...
    console.error('restore-inventory error:', JSON.stringify({
